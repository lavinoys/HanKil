		...
		<div class="contents" style="height: 470px;">
			<table class="detail02">
				...
				<tr>
					...
					<td style="width:900px;border-top:5px solid #ddd;border-bottom:5px solid #ddd;padding:5px;" colspan="5">${community_info.title}</td>
				</tr>
				<tr>
					<th style="width:124px;border-bottom:5px solid #ddd;padding:5px 0;" scope="row">작성자</th>
					<td style="border-bottom:5px solid #ddd;padding:5px 0;text-align:center;">${community_info.user_name}</td>
					<th style="width:124px;border-bottom:5px solid #ddd;padding:5px 0;" scope="row">작성일</th>
					<td style="border-bottom:5px solid #ddd;padding:5px 0;text-align:center;">${community_info.update_date}</td>
					<th style="width:124px;border-bottom:5px solid #ddd;padding:5px 0;" scope="row">조회</th>
					<td style="border-bottom:5px solid #ddd;padding:5px 0;text-align:left;">${community_info.count}</td>
				</tr>
				</tbody>
			</table>
			<c:choose>
				<c:when test="${empty requestScope.file_info}">
					<div style="padding-top:10px;">
	         			<textarea disabled cols="40" rows="10" style="resize:none;background-color:#fff;padding:10px;">${community_info.content}</textarea>
					</div>
				</c:when>
				<c:otherwise>
					<c:choose>
		         		<c:when test="${file_info.file_type == 'zip'}">
		         			<div style="padding-top:10px;">
								<iframe src="${pageContext.request.contextPath}${file_info.file_url}/index.html" style="width: 100%;height: 600px;">
		         		...
		         		<c:otherwise>
							...
							<div style="padding-top:10px;">
								<a href="${pageContext.request.contextPath}${file_info.file_url}" target="_blank">#첨부파일 : [ ${file_info.file_name} ]</a>
							</div>
		         		</c:otherwise>
		         	</c:choose>
				</c:otherwise>
